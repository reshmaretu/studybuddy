...
export const CanvasPresenceSidebar: React.FC<CanvasPresenceSidebarProps> = ({
  ...
  if (isCollapsed) {
    return (
      <button
        ...
        className={`fixed z-[450] flex items-center gap-2 rounded-2xl border border-white/10 bg-[#0b1211]/95 px-3 py-2 text-xs text-white/70 shadow-2xl backdrop-blur-xl hover:text-white transition-colors ${
          isMobile ? 'right-4 bottom-24' : 'right-4 top-28'
        }`}
  ...
  // Mobile: Show as drawer from bottom
  if (isMobile) {
    return (
      <>
        {/* Backdrop */}
        <button
          ...
          className="fixed inset-0 z-[440] bg-black/40 backdrop-blur-sm"
          aria-label="Close presence drawer"
        />
        
        {/* Drawer */}
        <div className="fixed left-4 right-4 bottom-24 z-[450] rounded-3xl border border-white/10 bg-[#0b1211]/95 p-4 shadow-2xl backdrop-blur-xl max-h-[50vh]">
  ...
  return (
    <div className="hidden md:block fixed right-6 top-24 z-[450] w-64 rounded-3xl border border-white/10 bg-[#0b1211]/95 p-4 shadow-2xl backdrop-blur-xl">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2 text-xs uppercase tracking-widest text-white/60">
          <Users size={14} /> Presence ({participants.length})
        </div>
