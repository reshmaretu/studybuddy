// 🛠️ DEV-ONLY ROOM ASSETS
// These are only injected when 'Enable Dev-Only Room Options' is toggled in the DevOverlay.

export const DEV_AUDIO_TRACKS = [
    { name: 'Propane Nightmares', pro: true, url: '/assets/audio/propane_nightmares.mp3' },
    { name: 'Dev: Cybernetic Pulse', pro: true, url: '/assets/audio/dev_cybernetic_pulse.ogg' },
    { name: 'Dev: Glitch Static', pro: true, url: '/assets/audio/dev_glitch_static.ogg' }
];

export const DEV_LIVE_BGS = [
    { name: 'Tomoe Mami (Live)', pro: true, asset: 'Tomoe Mami', url: '/assets/bgs/live/tomoe_mami.mp4' },
    { name: 'Dev: Neon Circuit', pro: true, asset: 'Neon Circuit', url: '/assets/bgs/live/dev_neon_circuit.mp4' }
];

export const DEV_STATIC_BGS = [
    { name: 'Tomoe Mami (Static)', pro: true, url: '/assets/bgs/static/tomoe_mami.jpg' },
    { name: 'Dev: Rain Window', pro: true, url: '/assets/bgs/static/dev_rain_window.jpg' }
];
